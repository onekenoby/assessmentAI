...
def run_pending_jobs(max_jobs: int = 1) -> Dict[str, Any]:
    """Elabora fino a ``max_jobs`` job, in modo seriale e tenant-safe."""
    ...
    if PG_SCHEMA_MIGRATION_ONLY:
        raise RuntimeError(
            "PG_SCHEMA_MIGRATION_ONLY=1: la API non può eseguire job di ingestion"
        )
    started = time.time()
    ...
    status = "DONE" if failed == 0 else ("PARTIAL_FAILED" if done else "FAILED")
    ...
    return {
        ...
        "jobs_completed": done,
        "jobs_failed": failed,
        "queue_empty": len(results) < max_jobs,
        "processing_time_ms": _ms(started),
        "jobs": results,
    }
