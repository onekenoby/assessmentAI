"""Entry point ASGI della Ingestion API.
...
from api.routes_ingestion import REQUEST_ID_HEADER, health_router, router
    ...
    try:
        if settings.initialize_on_startup:
            try:
                ...
                logger.exception("Startup ingestion fallito")
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="Multi-Tenant Ingestion API",
        version="1.0.0",
        description=(
            "Orchestrazione tenant-safe della pipeline BYTEA -> PostgreSQL, "
            "Qdrant e Neo4j. I metadati autorizzativi provengono esclusivamente "
            "dal Database A tramite le worker API rag_ingestion."
        ),
        lifespan=lifespan,
    )
