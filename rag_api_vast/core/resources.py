...
class ResourceManager:
    ...
    def initialize(self, *, strict: bool = True, force: bool = False) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                if self._config.ollama_warmup_on_startup:
                    self._warmup_ollama(local["ollama_session"])
    ...
    def _warmup_ollama(self, session: Any) -> None:
        """Carica il modello remoto in memoria prima della prima query RAG."""

        if session is None:
            raise ResourceNotReadyError("Sessione Ollama assente durante il warm-up")

        logger.info(
            "Warm-up Ollama avviato | endpoint=%s model=%s keep_alive=%s",
            self._config.ollama_native_chat_url,
            self._config.llm_model_name,
            self._config.ollama_request_keep_alive,
        )
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "Rispondi soltanto con OK.",
                }
            ],
            "stream": False,
            "think": False,
            "keep_alive": self._config.ollama_request_keep_alive,
            "options": {
                "temperature": 0.0,
                "num_ctx": min(self._config.llm_num_ctx, 2048),
                "num_predict": 8,
        ...
        response = session.post(
            ...
            timeout=(
                self._config.ollama_connect_timeout_seconds,
                self._config.ollama_warmup_timeout_seconds,
            ),
        )
        response.raise_for_status()
        body = response.json()
        if not isinstance(body, Mapping) or body.get("error"):
            detail = body.get("error") if isinstance(body, Mapping) else "payload non valido"
            raise ResourceInitializationError(
                f"Warm-up Ollama non valido: {str(detail)[:500]}"
            )
        logger.info("Warm-up Ollama completato | model=%s", self._config.llm_model_name)
