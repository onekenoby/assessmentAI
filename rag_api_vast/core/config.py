...
class RagSettings(BaseModel):
    ...
    # ------------------------------------------------------------------
    # Ollama
    # ``ollama_base_url`` è l'endpoint unico, locale o remoto, compatibile con
    # il meccanismo usato dalla Ingestion API. Gli URL specifici possono essere
    # ancora sovrascritti separatamente per retrocompatibilità.
    # ------------------------------------------------------------------
    ...
    ollama_warmup_timeout_seconds: int = 600
    ...
    @classmethod
    def validate_ollama_urls(cls, value: str) -> str:
        cleaned = value.strip().rstrip("/")
        if not cleaned.startswith(("http://", "https://")):
            raise ValueError("gli endpoint Ollama devono usare http:// o https://")
        return cleaned

    @field_validator("ollama_request_keep_alive")
    @classmethod
    def validate_keep_alive(cls, value: str) -> str:
        cleaned = value.strip()
        if not cleaned:
            raise ValueError("OLLAMA_REQUEST_KEEP_ALIVE non può essere vuoto")
        return cleaned
...
def load_settings() -> RagSettings:
    ...
    settings = RagSettings(
        ...
        ollama_warmup_on_startup=_env_bool("OLLAMA_WARMUP_ON_STARTUP", False),
        ollama_warmup_timeout_seconds=_env_int(
            "OLLAMA_WARMUP_TIMEOUT_S", 600
        ),
        ollama_request_keep_alive=_env_str(
            "OLLAMA_REQUEST_KEEP_ALIVE", "30m"
        ),
