...
def _normalize_docx_text(value: str) -> str:
    """Normalizza il testo Word senza alterarne il contenuto semantico."""
...
def _escape_markdown_table_cell(value: str) -> str:
    value = _normalize_docx_text(value)
    value = value.replace("|", r"\|")
    return value.replace("\n", "<br>")


def _docx_heading_level(paragraph: Paragraph) -> Optional[int]:
    style_name = str(
...
def _docx_is_list_paragraph(paragraph: Paragraph) -> bool:
    style_name = str(
        getattr(getattr(paragraph, "style", None), "name", "") or ""
    ).lower()
...
def _docx_image_extension(content_type: str, fallback_name: str) -> str:
    ...
    suffix = os.path.splitext(str(fallback_name or ""))[1].lower()
...
def _extract_docx_paragraph_images(
    ...
    for blip in paragraph._p.xpath(".//a:blip"):
        ...
        if image_hash in image_cache:
            image_path = image_cache[image_hash]
        else:
            os.makedirs(assets_dir, exist_ok=True)
            extension = _docx_image_extension(
                getattr(image_part, "content_type", ""),
                str(getattr(image_part, "partname", "")),
            )
            image_name = f"image_{len(image_cache) + 1:03d}{extension}"
            image_path = os.path.join(assets_dir, image_name)
            with open(image_path, "wb") as image_file:
                image_file.write(image_bytes)
            image_cache[image_hash] = image_path

        relative_path = os.path.relpath(image_path, markdown_dir)
        markdown_images.append(
            f"![DOCX embedded image]({Path(relative_path).as_posix()})"
        )
...
def _docx_table_to_markdown(table: Table) -> str:
    ...
    for row in table.rows:
        values = [
            _escape_markdown_table_cell(cell.text)
...
def convert_docx_to_markdown(
    ...
    """
    Converte deterministicamente un DOCX in Markdown temporaneo.

    Preserva titoli, paragrafi, liste, tabelle e immagini embedded.
    Header e footer Word non vengono inclusi, evitando duplicazioni per pagina.
    """
    docx_path = os.path.abspath(docx_path)
    if markdown_path is None:
        markdown_path = os.path.join(
            os.path.dirname(docx_path),
            f"{Path(docx_path).stem}.converted.md",
        )
    markdown_path = os.path.abspath(markdown_path)
    markdown_dir = os.path.dirname(markdown_path)
    assets_dir = os.path.join(
        markdown_dir,
        f"{Path(markdown_path).stem}_assets",
    )
    ...
    for child in document.element.body.iterchildren():
        if child.tag == qn("w:p"):
            ...
            if text:
                ...
                elif _docx_is_list_paragraph(paragraph):
                    paragraph_blocks.append(f"- {text}")
                else:
                    paragraph_blocks.append(text)

            paragraph_blocks.extend(
                _extract_docx_paragraph_images(
                    paragraph=paragraph,
                    document=document,
            ...
            table_markdown = _docx_table_to_markdown(
    ...
    print(
        f"   📝 DOCX convertito in Markdown: "
        f"{os.path.basename(markdown_path)} | "
        f"images={len(image_cache)}"
    )
...
def extract_docx_chunks(
    ...
    """Converte il DOCX in Markdown e riusa l'estrattore Markdown attivo."""
    original_filename = os.path.basename(file_path)
    markdown_path = convert_docx_to_markdown(file_path)
    ...
    for chunk in chunks:
        ...
        metadata["intermediate_format"] = "markdown"
        metadata["conversion"] = "docx_to_markdown"

    print(
        f"   📄 DOCX Ingested: {len(chunks)} chunk validi."
    )
    return chunks
...
def suffix_from_db_payload(payload: Dict[str, Any]) -> str:
    ...
    raise ValueError(
        ...
        "Sono ammessi PDF, Markdown e DOCX."
    )
...
def extract_db_document_chunks(
    ...
    if file_path.lower().endswith(".docx"):
        return extract_docx_chunks(file_path, log_id)
    if file_path.lower().endswith(".pdf"):
